...
mcmc_results <- ABC_sequential(
  method = "Drovandi",                          # SMC method
  model = model,                                # Your model
  prior = prior,                                # Your prior
  nb_simul = 100000,                            # Total number of simulations
  summary_stat_target = c(mean(observed_data), sd(observed_data)),  # Target summary stats
  tolerance_tab = c(1, 0.5),                      # Tolerance at each stage (1 stage here)
  use_seed = FALSE,                             # Reproducibility
